        ...
        switch(sortTypeEnum){
            case BY_NAME:
                comparator = (path1, path2)-> {
                    return ((new File(path1)).getName().toUpperCase().compareTo((new File(path2)).getName().toUpperCase()));
                };
                break;
            case BY_DATE:
                comparator = (path1, path2)->{
        ...
        switch(sortTypeEnum){
            case BY_NAME:
                comparator = (mediaFile1, mediaFile2)->{
                    return (mediaFile1.getFileName().toUpperCase().compareTo(mediaFile2.getFileName().toUpperCase()));
                };
                break;
            case BY_DATE:
                comparator = (mediaFile1, mediaFile2)->{
            ...
            case BY_DURATION:
                comparator = (mediaFile1, mediaFile2)->{
                    return (Integer.compare(mediaFile1.getFileDuration(),
