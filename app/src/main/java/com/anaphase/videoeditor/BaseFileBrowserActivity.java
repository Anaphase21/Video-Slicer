...
public class BaseFileBrowserActivity extends AppCompatActivity implements AlertDialogBox.AlertDialogListener {
    ...
    private TimeUnit TIME_UNIT = TimeUnit.MILLISECONDS;
        ...
        handler = new Handler(Looper.getMainLooper()){
            ...
            public void handleMessage(Message message){
                ...
                if((mediaFilePosition > -1) && (recyclerViewAdapter != null)){
                    ...
                    //long numDirectories = mediaFiles.stream().filter((e)->e.isDirectory()).count();
                        ...
                        if(BaseFileBrowserActivity.this instanceof FileBrowserActivity){
                            ...
                            if(count != null){
                                if((count == mediaFilesOnlyCount) && (fba.getCurrentDirectory().equals(fileParentDirectory))){
                                    System.out.println("CURRENT FILE SIZE ======== "+count);
                                    enableMenuItems();
                                }
                            }
                    ...
                    /*if(fileLoaded == 0){
                        if((++numFilesFullyLoaded) == (mediaFiles.size() - numDirectories)){
                            enableMenuItems();
                            resetNumFilesFullyLoaded();
                        }else{
                            System.out.println("File #: "+numFilesFullyLoaded);
                        }
                    }**/
                }
            }
            ...
            public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
                switch(item.getItemId()){
                    case shareItemId:
                        ...
                        //MimeTypeMap.getFileExtensionFromUrl(filesToShare.get(0).toString());
                        intent.setType(mimeTypeMap.getMimeTypeFromExtension(extension));
                        //System.out.println("=============="+mimeTypeMap.getMimeTypeFromExtension(extension)+"==============");
